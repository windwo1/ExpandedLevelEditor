    ...
    def execute(self, context):
        ...
        for element in root.iter():
            ...
            if settings is not None:
                terrain_decimation = parse_float(settings.findtext('TerrainDecimation'))
                break
        ...
        for filename in os.listdir(lights_folder):
            ...
            for light_data in root.findall('.//PbrSphereLightEntityData'):
                ...
                light.energy = float(intensity_node.text) if intensity_node is not None else 1.0
        ...
        def create_meshes(xml_file_path, material_xml_path):
            ...
            for element in root.iter():
                ...
                if settings is not None:
                    ...
                    emissive_materials = settings.findtext('UseEmission') == "True"
                        ...
                        for name in names:
                            ...
                            if material_name in bpy.data.materials:
                                ...
                                if mesh_material:
                                    ...
                                    asm = texture_params.get("ASM")
                                    color = texture_params.get("Color")
                                    ett = texture_params.get("ETT")
                                    normal = texture_params.get("Normal")

                                    asm_path = os.path.join(textures_folder, f"{asm}.png")
                                    color_path = os.path.join(textures_folder, f"{color}.png")
                                    ett_path = os.path.join(textures_folder, f"{ett}.png")
                                    ...
                                    asm_texture_node = material.node_tree.nodes.new("ShaderNodeTexImage")
                                    asm_texture_node.location = (-800, 100)

                                    asm_seperate_node = material.node_tree.nodes.new("ShaderNodeSeparateColor")
                                    asm_seperate_node.location = (-400, 100)
                                    ...
                                    ett_texture_node = material.node_tree.nodes.new("ShaderNodeTexImage")
                                    ett_texture_node.location = (-400, -500)

                                    ett_seperate_node = material.node_tree.nodes.new("ShaderNodeSeparateColor")
                                    ett_seperate_node.location = (50, -500)
                                    ...
                                    if os.path.exists(asm_path) and asm:
                                        asm_texture_node.image = load_texture(asm_path)
                                        asm_texture_node.image.colorspace_settings.name = "Non-Color"

                                        material.node_tree.links.new(asm_texture_node.outputs["Color"], asm_seperate_node.inputs["Color"])
                                        material.node_tree.links.new(asm_seperate_node.outputs["Red"], ao_mix_node.inputs["Factor"])
                                        material.node_tree.links.new(asm_seperate_node.outputs["Green"], invert_smoothness_node.inputs["Color"])
                                        material.node_tree.links.new(asm_seperate_node.outputs["Blue"], bsdf.inputs["Metallic"])
                                    ...
                                    if os.path.exists(color_path) and color:
                                    ...
                                    if os.path.exists(ett_path) and ett and emissive_materials:
                                        ett_texture_node.image = load_texture(ett_path)
                                        bsdf.inputs["Emission Strength"].default_value = 1.0

                                        material.node_tree.links.new(ett_texture_node.outputs["Color"], ett_seperate_node.inputs["Color"])
                                        material.node_tree.links.new(ett_seperate_node.outputs["Red"], bsdf.inputs["Emission Color"])
