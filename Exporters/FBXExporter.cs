...
namespace LevelEditorPlugin.Exporters
{
    public class FBXExporter
    {
        ...
        /// </summary>
        public void ExportFBX(dynamic meshAsset, string filename, string fbxVersion, string units, int lodIndex, string fileType, params MeshSet[] meshSets)
        {
            using (FbxManager manager = new FbxManager())
            {
                ...
                if (meshSets[0].Lods[0].Type == MeshType.MeshType_Composite)
                {
                ...
                foreach (MeshSet meshSet in meshSets)
                {
                    if (meshSet.Lods.Count - 1 < lodIndex)
                    {
                        lodIndex = meshSet.Lods.Count - 1;
                    }
